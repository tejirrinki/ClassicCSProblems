...
class Maze:
	def __init__(self,rows:int = 10, columns:int = 10,sparsness:float = 0.2,start:MazeLocation = MazeLocation(0,0),goal:MazeLocation= MazeLocation(9,9)) -> None:
		...
		#self.sparsness:float = sparsness
...
if solution1 is None:
	print("No solution found using depth-firsth search")
else:
...
if solution2 is None:
	...
	path2:List[MazeLocation] = node_to_path(solution1)
	m.mark(path2)
	print(m)
	m.clear(path2)	
...
if solution3 is None:
	...
	path3:List[MazeLocation] = node_to_path(solution1)
	m.mark(path3)
	print(m)
	m.clear(path3)	
